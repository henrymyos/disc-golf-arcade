...
import { PLACEMENT_MIN_RATING, PLACEMENT_MAX_RATING, PLACEMENT_EDGE } from "./ranked";
...
// The placement (calibration) field for one of your first ranked rounds: a field
// spread EVENLY across the rating band [lo, hi], so where you finish maps straight
// back onto that band (see applyPlacementRound). Round 1 spans the whole ladder
// (40→90); later rounds span your PROJECTED division (its tier mean ±14), so you're
// tested at your level. The bots carry PLACEMENT_EDGE — they out-play their rating
// by a few strokes — so a rank has to be earned rather than coasted into.
export function rankedPlacementField(seed: number, size: number, roundHoles: Hole[], lo = PLACEMENT_MIN_RATING, hi = PLACEMENT_MAX_RATING): FieldPlayer[] {
  const par = roundHoles.reduce((s, h) => s + h.par, 0);
  const diff = roundHoles.map(holeDifficulty);
  const rng = mulberry32((seed ^ 0x5bd1e995) >>> 0);
  const field: FieldPlayer[] = [];
